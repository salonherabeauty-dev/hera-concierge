...
function renderConversationDrawer(detail) {
    ...
    const latestCandidate = latestInbound
        ? detail.candidates.find((candidate) => candidate.sourceMessageId === latestInbound.id)
        : undefined;
    const previousCandidate = detail.candidates.find((candidate) => candidate.sourceMessageId !== latestInbound?.id);
    ...
    const latestJobClass = latestJob?.status === "completed"
        ? "pill--normal"

...
function isApprovedPreviewOrigin(url: URL): boolean {
  return (
...
export function websiteConciergeOrigin(
  ...
  try {
    ...
    if (isApprovedPreviewOrigin(url)) return url.origin;
    return null;
  } catch {
    return null;
  }
...
export function requireAllowedWebsiteOrigin(
  ...
  const raw = firstHeader(request.headers.origin);
  if (!raw && process.env.VERCEL_ENV !== "production") return;
  if (!raw || !websiteConciergeOrigin(request)) {

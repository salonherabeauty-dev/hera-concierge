...
if (!rootElement) throw new Error("Command centre root element was not found");
...
function taskBadge(task: Task): string {
  ...
  if (task.overdue) classes.push("pill--overdue");
...
function modeBadge(mode: "ai" | "management"): string {
...
function canAcceptTask(task: Task): boolean {
  const role = state.staff?.role ?? "auditor";
  if (role === "owner" || role === "managing_director") return true;
  if (role === "salon_manager") return !privacyTaskTypes.has(task.taskType);
  if (role === "receptionist") return receptionTaskTypes.has(task.taskType);
  if (role === "technical_lead") return technicalTaskTypes.has(task.taskType);
  if (role === "finance_admin") return task.taskType === "refund_finance";
  if (role === "privacy_officer") return privacyTaskTypes.has(task.taskType);
  return false;
}

function canTransitionTask(task: Task): boolean {
  const role = state.staff?.role ?? "auditor";
  if (["owner", "managing_director", "salon_manager"].includes(role)) return true;
  return Boolean(state.staff && task.ownerUserId === state.staff.userId && canAcceptTask(task));
}

function canControlConversation(): boolean {
  return [
    "owner",
    "managing_director",
    "salon_manager",
...
function setNotice(type: "success" | "error" | "info", message: string): void {
...
function loadingBlock(label = "Loading command centre"): string {
...
function navButton(view: ViewName, icon: string): string {
...
function metricCard(label: string, value: number, note: string, tone = "neutral"): string {
...
function renderOverview(): string {
  ...
  if (!dashboard) return emptyState("No operational snapshot", "Refresh to load the latest command centre state.");
...
function conversationRow(conversation: Conversation): string {
...
function renderQuality(): string {
  ...
  if (!quality) return emptyState("Quality evidence unavailable", "Refresh the command centre to load the latest evidence.");
...
root.addEventListener("submit", (event) => {
  ...
  if (form.id === "note-form") {
    ...
    void commandApi
      ...
      .catch((error: unknown) => setNotice("error", error instanceof Error ? error.message : "The note could not be saved."))
      .finally(() => {
        state.busy = false;
        render();
      });
...
root.addEventListener("click", (event) => {
  ...
  const viewControl = target.closest<HTMLElement>("[data-view]");
  if (viewControl?.dataset.view) {
    state.selected = null;
    void loadView(viewControl.dataset.view as ViewName);
    return;
  }
  const filterControl = target.closest<HTMLElement>("[data-task-filter]");
  ...
  if (action === "dismiss-notice") {
    ...
    void commandApi
      ...
      .catch((error: unknown) => setNotice("error", error instanceof Error ? error.message : "The task could not be accepted."))
      .finally(() => {
        state.busy = false;
        render();
      });
    ...
    const taskId = actionControl.dataset.taskId ?? "";
    const version = Number(actionControl.dataset.version);
    if (!window.confirm("Mark this human-action task as resolved? The audit trail will be retained.")) return;
    state.busy = true;
    void commandApi
      ...
      .catch((error: unknown) => setNotice("error", error instanceof Error ? error.message : "The task could not be resolved."))
      .finally(() => {
        state.busy = false;
        render();
      });
    ...
    void commandApi
      ...
      .catch((error: unknown) => setNotice("error", error instanceof Error ? error.message : "Takeover could not be activated."))
      .finally(() => {
        state.busy = false;
        render();
      });
    ...
    void commandApi
      ...
      .catch((error: unknown) => setNotice("error", error instanceof Error ? error.message : "The conversation could not return to AI."))
      .finally(() => {
        state.busy = false;
        render();
      });

...
const EXPECTED_BRANCH = "diag/stage0-runtime-readiness";
...
if (process.env.VERCEL_ENV !== "preview") {
  throw new Error("stage0_audit_requires_preview");
}
if (process.env.VERCEL_GIT_COMMIT_REF !== EXPECTED_BRANCH) {
  throw new Error("stage0_audit_requires_isolated_diagnostic_branch");
}
...
if (operations.sendMode !== "shadow") {
  throw new Error("stage0_audit_requires_shadow_mode");
}
if (process.env.WHATSAPP_LIVE_CONFIRMATION === "ENABLE_HERA_WHATSAPP_LIVE") {
  throw new Error("stage0_audit_refuses_live_confirmation");
}
...
const client = createClient(database.url, database.serviceRoleKey, {
  ...
  global: { headers: { "X-Client-Info": "hera-stage0-readiness-audit" } },
});

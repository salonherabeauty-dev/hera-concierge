const expectedBranch = "ops/stage3r-activation";

if (process.env.VERCEL_ENV !== "preview") {
  throw new Error("Stage 3-R activation probe requires Vercel Preview");
}
if (process.env.VERCEL_GIT_COMMIT_REF !== expectedBranch) {
  throw new Error("Stage 3-R activation probe requires the isolated activation branch");
}
if ((process.env.WHATSAPP_SEND_MODE ?? "shadow") !== "shadow") {
  throw new Error("Stage 3-R activation probe requires WhatsApp shadow mode");
}
if (process.env.WHATSAPP_LIVE_CONFIRMATION === "ENABLE_HERA_WHATSAPP_LIVE") {
  throw new Error("Stage 3-R activation probe refuses a live confirmation");
}

...
const DIAGNOSTIC_VERSION = "pr71-build-proof-1.0.0";
...
async function main(): Promise<void> {
  if (!shouldRun()) {
    console.log(JSON.stringify({
      ...
      state: "skipped_outside_expiring_shadow_preview",
    }));
    return;
  }
  ...
  const database = createClient(
    ...
    databaseConfig.serviceRoleKey,
    {
      ...
      global: { headers: { "X-Client-Info": DIAGNOSTIC_VERSION } },
    },
  );
  const runtime = createProductionRuntime();
  if (runtime.sendMode !== "shadow") {
    throw new Error("pr71_proof_runtime_not_shadow_locked");
  }

  // The proof must create reviewable candidates but must not claim or deliver
  // any outbox item. The normal Reception Desk remains the only human-approved
  // delivery path after this diagnostic build.
  const repositoryWithSuppressedOutbox = runtime.repository as typeof runtime.repository & {
    claimOutbox: (...args: unknown[]) => Promise<never[]>;
  };
  repositoryWithSuppressedOutbox.claimOutbox = async () => [];

  const { data: recentJobs, error: recentJobsError } = await database
    .from("ai_jobs")
    .select("id,source_message_id,status,attempts,max_attempts,payload,created_at")
    .gte("created_at", "2026-08-29T00:00:00.000Z")
    .order("created_at", { ascending: false })
    .limit(500);
  if (recentJobsError) throw new Error("pr71_proof_jobs_unavailable");

  const failures: string[] = [];
  const results: Record<string, unknown>[] = [];

  for (const target of TARGETS) {
    const job = (recentJobs ?? []).find(
      (candidate) =>
        hash(String(candidate.id)) === target.jobHash &&
        hash(String(candidate.source_message_id)) === target.sourceHash,
    );
    if (!job) {
    ...
    const { data: priorAttempts } = await database
      .from("ai_audit_log")
      .select("id")
      .eq("event_type", "pr71_build_proof_requested")
    ...
    const { data: existingCandidate } = await database
      .from("ai_outbox")
      .select("id,status,provider_message_id,body,created_at")
    ...
    if (!existingCandidate && (priorAttempts?.length ?? 0) >= 1) {
    ...
    if (!existingCandidate) {
      ...
      const { error: resetError } = await database
        ...
        .update({
          ...
          payload: {
            ...record(job.payload),
            diagnosticRun: DIAGNOSTIC_VERSION,
      ...
      const { error: auditError } = await database.from("ai_audit_log").insert({
        ...
        actor_id: DIAGNOSTIC_VERSION,
        event_type: "pr71_build_proof_requested",
    ...
    const { data: finalJob, error: finalJobError } = await database
      .from("ai_jobs")
      .select("status,attempts,max_attempts,last_error,completed_at")
      .eq("id", jobId)
      .maybeSingle();
    const { data: candidate, error: candidateError } = await database
      ...
      .limit(1)
      .maybeSingle();
    const { data: policyDecision, error: policyError } = await database
      .from("ai_decisions")
      .select("model_id,prompt_version,policy_version,output,created_at")
      .eq("source_message_id", sourceMessageId)
      .eq("stage", "policy")
    ...
    const text = candidateText(candidate?.body);
    ...
    await database.from("ai_audit_log").insert({
      ...
      actor_id: DIAGNOSTIC_VERSION,
      event_type: passed
        ? "pr71_build_proof_passed"
        : "pr71_build_proof_blocked",
      target_type: "diagnostic_target",
      target_id: target.key,
      details: {
        ...result,
  ...
  if (failures.length > 0) {
    throw new Error(`pr71_build_proof_blocked:${failures.join(",")}`);
  }
}

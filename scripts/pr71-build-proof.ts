...
const EXPIRES_AT = Date.parse("2026-08-30T04:30:00.000Z");
const DIAGNOSTIC_VERSION = "pr71-staging-proof-1.0.0";
...
async function main(): Promise<void> {
  ...
  // Create review candidates only. Do not claim any outbox item, contact any
  // WhatsApp provider or convert a candidate to a delivered message during proof.
  ...
  for (const target of TARGETS) {
    ...
    let proofStartedAt: string | null = null;
    if (!existingCandidate) {
      const { data: priorAttempts } = await database
        .from("ai_audit_log")
        .select("id")
        .eq("event_type", "pr71_staging_proof_requested")
        .eq("target_type", "diagnostic_target")
        .eq("target_id", target.key)
        .limit(2);
      if ((priorAttempts?.length ?? 0) >= 1) {
      ...
      const { error: resetError } = await database
        ...
        .update({
          ...
          attempts: 0,
          max_attempts: 1,
          available_at: proofStartedAt,
          locked_at: null,
          locked_by: null,
          last_error: null,
          completed_at: null,
          updated_at: proofStartedAt,
          payload: {
            ...record(job.payload),
      ...
      const { error: auditError } = await database.from("ai_audit_log").insert({
        ...
        event_type: "pr71_staging_proof_requested",
    ...
    let policyQuery = database
      ...
      .eq("source_message_id", sourceMessageId)
      .eq("stage", "policy");
    if (proofStartedAt) policyQuery = policyQuery.gte("created_at", proofStartedAt);
    ...
    await database.from("ai_audit_log").insert({
      ...
      actor_id: DIAGNOSTIC_VERSION,
      event_type: passed
        ? "pr71_staging_proof_passed"
        : "pr71_staging_proof_blocked",
      target_type: "diagnostic_target",
      target_id: target.key,
      details: {
        ...result,
  ...
  if (failures.length > 0) {
    throw new Error(`pr71_staging_proof_blocked:${failures.join(",")}`);
  }
}

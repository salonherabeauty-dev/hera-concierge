...
if (process.env.VERCEL_ENV !== "preview") throw new Error("diagnostic_requires_preview");
if (process.env.VERCEL_GIT_COMMIT_REF !== EXPECTED_BRANCH) {
  throw new Error("diagnostic_requires_authoritative_staging_branch");
}
if (process.env.WHATSAPP_SEND_MODE !== "shadow") throw new Error("diagnostic_requires_shadow_mode");
...
if (!messages?.length) throw new Error("inbound_messages_not_found");
const target = messages.find((item) => String(item.text_body).includes(TARGET_FRAGMENT));
if (!target) throw new Error("fresh_quality_gate_message_not_found");
const relevantMessages = messages
  .filter((item) => Date.parse(item.created_at) >= Date.parse(target.created_at))
  .sort((left, right) => Date.parse(left.created_at) - Date.parse(right.created_at));
const relevantIds = relevantMessages.map((item) => item.id);

const [jobResult, decisionResult, outboxResult, taskResult] = await Promise.all([
  supabase.from("ai_jobs").select("*").in("source_message_id", relevantIds).order("created_at"),
  supabase
    .from("ai_decisions")
    .select("id,source_message_id,stage,model_id,prompt_version,policy_version,risk,confidence,created_at,latency_ms")
    .in("source_message_id", relevantIds)
    .order("created_at"),
  supabase.from("ai_outbox").select("*").in("source_message_id", relevantIds).order("created_at"),
  supabase
    ...
    .eq("conversation_id", conversation.id)
    .gte("created_at", new Date(Date.parse(target.created_at) - 120_000).toISOString())
    .order("created_at"),
]);
...
console.log(
  "HERA_FRESH_COMPLAINT_SEQUENCE_DIAGNOSTIC",
  JSON.stringify({
    contact: { phoneEnding: String(contact.wa_id).slice(-4), profileName: contact.profile_name },
    conversation,
    targetMessageId: target.id,
    inboundSequence: relevantMessages.map((item) => ({
      id: item.id,
      kind: item.kind,
      createdAt: item.created_at,
      providerTimestamp: item.provider_timestamp,
      textPreview: safeText(item.text_body, 220),
    })),
    jobs,
